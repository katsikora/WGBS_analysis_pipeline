    ...
    with open(os.path.join(metDir,"logs","%s.mbias.out" % read_root),'w') as stdoutF, open(os.path.join(metDir,"logs","%s.mbias.err" % read_root),'w') as stderrF:
        try:
            stdout_res, stderr_res  = run_job(cmd_str       = Mb_cmd,
                                          ...
                                          run_locally       = False,
                                          working_directory = os.getcwd(),
                                          job_other_options = '-p bioinfo --mem-per-cpu=3000 --mincpus='+str(nthreads))
...
def BS_QC_rep(Rpath,QCdir,logobject):
    ...
    cmd = os.path.join(Rpath, 'Rscript -e "rmarkdown::render(\'{0}/WGBS_QC_report_template.Rmd\', params=list(QCdir=\'{0}\'), output_file=\'{0}/QC_report.pdf\')"'.format(os.path.dirname(os.path.realpath(__file__))))
    logobject.info(cmd)
