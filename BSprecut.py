...
def cut_reads_auto(INfile1,INfile2,OUTfile1,OUTfile2,cutThdR1,cutThdR2,cutpath,my_session,cutout,logobject,args):
    read_root=re.sub('_R1.fastq.gz','',os.path.basename(INfile1))
    bshcmd=cutpath +' cutadapt -a AGATCGGAAGAGC -A AGATCGGAAGAGC --minimum-length 30  -n 5 -j' + str(args.nthreads) +' -u ' + cutThdR1 + ' -U ' + cutThdR2 + ' -o ' + OUTfile1 + ' -p ' + OUTfile2 + ' ' + INfile1 + ' ' + INfile2 + ';sleep 300'
    with open(os.path.join(cutout,"logs","%s.trim_reads.out" % read_root),'w+') as stdoutF, open(os.path.join(cutout,"logs","%s.trim_reads.err" % read_root),'w+') as stderrF:
        try:
            ...
            stdout_res, stderr_res  = run_job(cmd_str           = bshcmd,
                                      ...
                                      job_other_options = '-p bioinfo --nodes=1=1 --mincpus={}'.format(args.nthreads))
...
def cut_reads_user(INfile1,INfile2,OUTfile1,OUTfile2,cutpath,my_session,cutout,logobject, args):
    ...
    bshcmd = "{} cutadapt -a {} -A {} -q {} -m 30 -j {} {} -o {} -p {} {} {} ; sleep 300".format(cutpath,
                                                                            adapterSeq,
                                                                            adapterSeq,
                                                                            args.trimThreshold,
                                                                            args.nthreads,
