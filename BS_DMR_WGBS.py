...
def DMR_metilene(ii,sampleInfo,outfile,nthreads,metipath,my_session,logobject):
    ...
    met_cmd=os.path.join(metipath,'metilene')+ ' -a ' + list(set(pandas.read_table(args.sampleInfo)['Group']))[1] + ' -b ' + list(set(pandas.read_table(args.sampleInfo)['Group']))[2] + ' -t ' + str(nthreads) + ' ' + ii + ' | sort -k 1,1 -k2,2n > ' + outfile
...
def clean_up_metilene(metilene_out,CpG_stats_out,args.sampleInfo,outdir,my_session,logobject):
    cmd='/package/R-3.3.1/bin/Rscript --no-save --no-restore /data/manke/repository/scripts/DNA_methylation/WGBS_pipe/v0.02/WGBSpipe.metilene_stats.limma.R ' + outdir + ' ' + metilene_out + ' ' + CpG_stats_out +' ' + sampleSheet
    logobject.info(cmd)
